  ...
  if (view === 'itemDetail') {
    ...
    return (
      <div className="space-y-5">
        ...
        {activeTab === 'BOM usage' && (
          <div className="panel">
            <div className="panel-head"><h3>BOM / Recipe Usage</h3></div>
            <div className="panel-body p-0">
              <table>
                <thead>
                  <tr><th>BOM ID</th><th>Parent Product</th><th>Version</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {itemBoms.length > 0 ? (
                    itemBoms.map((b) => (
                      <tr key={b.id} onClick={() => onNavigate('bomDetail', { id: b.id })}>
                        <td className="cell-code">{b.id}</td>
                        <td className="font-semibold">{b.parentName}</td>
                        <td>{b.version}</td>
                    ...
                    <tr>
                      <td colSpan={4}><div className="empty-mini">Not referenced in any active BOM.</div></td>
                    </tr>
                  )}
                </tbody>
              </table>
    ...
    return (
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <button className="btn btn-sm btn-primary" onClick={openCreateBomModal}>
